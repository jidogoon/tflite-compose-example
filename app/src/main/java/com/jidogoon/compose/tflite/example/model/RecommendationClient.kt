...
import com.jidogoon.compose.tflite.example.model.FileUtil.loadGenreList
import com.jidogoon.compose.tflite.example.model.FileUtil.loadModelFile
import com.jidogoon.compose.tflite.example.model.FileUtil.loadMovieList
...
class RecommendationClient(private val context: Context, private val config: Config) {
    ...
    private fun loadCandidateList() {
        try {
            val collection: Collection<MovieItem> = loadMovieList(
                context.assets, config.movieList
            )
            candidates.clear()
    ...
    fun preprocess(selectedMovies: List<MovieItem>): Array<Any> {
        ...
        // Sort features.
        val sortedFeatures: List<Config.Feature> = ArrayList(config.inputs)
        sortedFeatures.sortedBy { it.index }
        for (feature in sortedFeatures) {
            when (feature.name) {
                Config.FEATURE_MOVIE -> {
                    inputs.add(preprocessIds(selectedMovies, feature.inputLength))
                }
                Config.FEATURE_GENRE -> {
                    inputs.add(preprocessGenres(selectedMovies, feature.inputLength))
                }
                else -> {
                    Log.e(TAG, java.lang.String.format("Invalid feature: %s", feature.name))
                }
            }
        }
